...
local function is_complete_law_code(code)
    return code:match("^[Dd]%d+$")
        or code:match("^[Dd]%d+%.%d+$")
        or code:match("^[Dd]%d+%.%d+%.%d+$")
        or code:match("^[Dd]%d+%.%.%d+$")
        or code:match("^[Dd]%d+[bvjkxyt]$")
        or code:match("^[Dd]%d+fb$")
end
...
function P.func(key, env)
    ...
    -- 法条模式中的句点是「条.款.项」层级分隔符，而不是标点。
    if (key_repr == "period" or key_repr == ".")
        and not key:ctrl() and not key:alt() and not key:super() and not key:shift()
    ...
    -- 分号全局选择当前页第二候选。直接选择可避免在 d/r 功能模式下
    -- 被下面的主键盘数字逻辑误当成编码「2」。
    if (key_repr == "semicolon" or key_repr == ";")
        and not key:ctrl() and not key:alt() and not key:super() and not key:shift()
        ...
        if composition and not composition:empty() then
            ...
            if menu and not menu:empty() then
                ...
                local page_start = math.floor(selected_index / page_sz) * page_sz
                local second_index = page_start + 1
                if context:select(second_index) then
                    return RIME_PROCESS_RESULTS.kAccepted
                end
            end
        end
    ...
    -- 引号全局选择当前页第三候选。
    if (key_repr == "apostrophe" or key_repr == "'")
        and not key:ctrl() and not key:alt() and not key:super() and not key:shift()
        and context:has_menu() then
        local composition = context.composition
        if composition and not composition:empty() then
            ...
            if menu and not menu:empty() then
                local selected_index = seg.selected_index or 0
                local page_start = math.floor(selected_index / page_sz) * page_sz
                local third_index = page_start + 2
                if context:select(third_index) then
                    return RIME_PROCESS_RESULTS.kAccepted
                end
            end
        end
    ...
    if r:match("^[0-9]$") then
        ...
        if env.has_menu then
            local select_keys = context:get_property("candidate_select_keys")
            if select_keys and select_keys ~= "" then
                -- 如果当前设置了候选选择键，且当前数字不在选择键内，则作为输入字符而非选词
                if not select_keys:find(r, 1, true) then
                    if context.push_input then
